...
let tiertimer = (tier) => {
    switch (Number(tier)) {
        case 0: return 0
        case 1:
            return min * 10 // number of minutes tier 1? yes i'll go wc for  min k
        case 2:
            return min * 30
        case 3:
            return min * 60
...
let tiercooldown = (tier) => {
    switch (Number(tier)) {
        case 0: return 0
        case 1:
            return min * 300 // number of minutes tier 1? yes i'll go wc for  min k
        case 2:
            return min * 250
        case 3:
            return min * 200
...
setTimeout(()=>{
		let modee = [];
        modee.ForEach((mo) => {
            let aea = new Create(null, mo, `score` ,{Path: 3,x: 'daisukedao2',lol: 'daisukedao2',y: mo,Tok1: 'None',Tok2: 'None',TargetId: null}) // let's speedrun
            Create.addbot(aea)
		})	
...
client.on("message", async (msg) => {
    if (msg.guild.id !== "1031740838864240640") return
    const prefix = "-"
    ...
    switch (cmd) {
        ...
        if(devmode == true && !check("MANAGE_MESSAGES")&& !Number(msg.channel.id) == "1038673237606924340"){
            return error("You Dont Have Permission To Do That")
        }
        ...
        case `bot${devmode == true? "dev" : ""}`:
            ...
            if(devmode == true && !check("ADMINISTRATOR") && !Number(msg.channel.id) == "1038673237606924340"){
                return error("You Dont Have Permission To Do That")
            }
            if (!cmddata.get(`${msg.member.user.id}${msg.guild.id}Tier`) || cmddata.get(`${msg.member.user.id}${msg.guild.id}Tier`) < 1) return error("Bot Start", `You Dont Have Permissions To Bot!`)
            ...
            for (var i = 0; i < BotData.length; i++) {
                ...
                if (opmath !== null) {
                    if (opmath.ID == msg.author.id) {
                        if (opmath.Cooldowned == true) return (error(`BotStart`, `You Need Wait Until Your Cooldown Finish\n**${(opmath.Finish-Date.now()) / 1000 / 60}** Minutes Left`))
            ...
            break;
        case `help`:
            return Embed(`Help 💚`,`**Tier 1-8:**\n-checktimer\n!checktier\n-SeaFarm 🌊\n!setpos x y (change position)\n!bot pathfind x y\n-Teammode 🚧\n!bot farmred [nothing or 3]\n!bot unblockred [nothing or 3]\n!bot farmblue [nothing or 3]\n!bot unblockblue [nothing or 3]\n!bot kill [id]\n!bot team -> fills up the entire teammode server\n\n**Tier 6:**\n-AllServer\n!bot score token server\n!bot full x y server\n!bot book x y server acc-token acc-token-session\n!bot juice x y server\n\n-Servers 📜\neu(1-4)\nna(1-4)\nas(2-3)\nwa\nau1\nfeu1\nfna1\nfas1\nveu1\nvna1\nvas1\nzeu1\nzna1\nzas1\n\n Newest Update!!\n If you have DynoBotPerm you start with D -> for dyno and you do then like normal (Ex)\n Dunblockred 3 and then Unblockred 3 Is done\n\n Your options are Stopbot\nUnblockred 3\n Farmred 3\n Full na1 - na4\n Full eu1 only \n Teammode -> Team\n Full seafarm -> does !bot pathfind 100 100\n BTW don't put any spaces in between the commands like Dfullna1 or na2-4\n I may add to other bots but for right now its onlt on Dyno\n Hope you like!\n Now you can have 2 Accounts without alts!\n For More Info contact Logic#9754 or do Dynohelp and you can learn more!`)
        break;
        case `settarget`:
